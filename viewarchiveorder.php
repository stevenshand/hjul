...
$orderListQuery = 
	...
	LEFT JOIN status ON archived_orders.status = status.id 
	LEFT JOIN models ON archived_orders.model = models.id 
	LEFT JOIN sizes ON archived_orders.size = sizes.id 
	LEFT JOIN fabrication_details ON archived_orders.id = fabrication_details.order_id 
	LEFT JOIN paint_details ON archived_orders.id = paint_details.order_id 
	LEFT JOIN assembly_details ON archived_orders.id = assembly_details.order_id 
	LEFT JOIN shipping ON archived_orders.id = shipping.order_id 
	LEFT JOIN address ON archived_orders.id = address.order_id 
	LEFT JOIN rohloff_serial ON archived_orders.id = rohloff_serial.orderId 
	WHERE archived_orders.id = ".$orderId;
...
$stmt->bind_result($orderDate, $targetWeek, $sname, $fname, $email, $frameNumber, $modelName, $brandId, $modelDisplayName, $frameSize, $orderStatus, $frameOnly, $statusId, $fabDetails, $paintDetails, $assemblyDetails, $basePrice, $vatExempt, $shippingNotes, $shippingDate, $shippingMethod, $trackingNumber, $tel1, $tel2, $rohloffSerial );
$stmt->store_result();
$stmt->fetch();
	...
	<div class="col-md-6">
		<div class="panel panel-default">
			<div class="panel-heading">
				<h3 class="panel-title">ARCHIVED ORDER DETAILS</h3>
			</div>
