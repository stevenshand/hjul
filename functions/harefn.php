...
//		// echo ( 'no CRM entry found<br>' );
//		$link = addCRMContact($emailAddress, $fname, $sname);
//	}else{
//		$contact = $response->body;
//		$link = $contact->{'profile-url'};
//		// echo ( 'CRM entry found, returning link....['.$link.']<br>' );
//	}
//
...
function addCRMContact($emailAddress, $fname, $sname){
	...
	// 	return null;
	//
	// $uri="https://api.hubapi.com/contacts/v1/contact?hapikey=".HUBSPOT_API_KEY;
	//
	// $contact = array(
	//             'properties' => array(
	//                 array(
...
function updateCRMLink($crmLink,$orderId){
	...
	// }
	//
	// $updateStatement = "update orders set crm_link = ? where id = ?";
	// if (!($stmt = $mysqli->prepare($updateStatement))) {
	//     echo "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error;
	// }
	//
	// if (!$stmt->bind_param("si", $crmLink, $orderId )) {
...
function fetchStockStatus( $orderId, &$rollingStockCount ){
	
	$bom = fetchBom($orderId);

    $stockStatus = [];
////	echo( $rollingStockCount[517]->qty );
//
//	$osstockItems = array();
//
//	foreach ($bom as $item) {
//		//echo( "[".$item->itemId."][".$item->qty."][".$rollingStockCount[$item->itemId]->qty."]<br>" );
//		$currQty = $rollingStockCount[$item->itemId]->qty;
//		if( $currQty < 1 ){
//			$osstockItems[] = $item->itemId;
//		}
//
//		$rollingStockCount[$item->itemId]->qty = $currQty-1;
//	}
//
//	// $instockItems = [1,3,9];
//	// $osstockItems = [20,36,48];
//	// $pendingStockItems = [200,336,488];
//
//	// $stockStatus->instockItems = $instockItems;
//    $stockStatus = [];
//    $stockStatus;
//	$stockStatus->osstockItems = $osstockItems;
//	// $stockStatus->pendingStockItems = $pendingStockItems;

	return $stockStatus;
}

function fetchBom( $orderId ){
...
function isRohloff($model, $frameonly){
	if( $frameOnly ){
		return "no";
	}
	
	if( strpos(strtolower($model), 'tumshie') !== false ){
		return "yes";
	}
	
	if (strpos(strtolower($model), 'rohloff') !== false) {
	    return 'yes';
	}else{
	    return 'no';
	}
