...
$insertStatement = "INSERT into fab_items(short_name, description, variation, supplier, supplier_code, cost, category ) values ( ?, ?, ?, ?, ?, ?, ? )";
if (!($stmt = $mysqli->prepare($insertStatement))) {
	$err = "err2";
}	

if (!$stmt->bind_param("sssssss", $item, $description, $variation, $supplier, $supplierCode, $cost, $category )) {
	$err = "err3";
}	
